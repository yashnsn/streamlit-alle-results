...
res_path = "./pinterest_base_image_gen_V5/pinterest_base_image_gen_V5"
src_path = "pinterest_images"
...
for ind_image in all_images:
    if not os.path.exists(os.path.join(res_path, f"{ind_image[:-4]}_upscaled.jpg")):
        continue
    
    metadata = json.load(open(os.path.join(res_path, f"{ind_image[:-4]}.json")))
    st.markdown(f'file name: {ind_image}\n\n score: {mapper[ind_image]:.02f}\n\n width: {metadata["width"]}, height: {metadata["height"]}\n\n time taken: {metadata["total time"]:.02f} seconds')
