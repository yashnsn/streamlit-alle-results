...
if selected_cat:
    ...
    for ind_file in all_files:
        ...
        # gen_img_V2_ref = Image.open(f'{root_path}/{ind_file}/output_text_desc_redux_switch_5_mask_V5_refined.jpg')
        ...
        if selected_cat=='Top':
            cols[2].write('mask for inpainting')
            cols[2].image(pin_ref_mask)
            cols[3].write('generated image(text+redux)')
            cols[3].image(gen_img)
        else:
            cols[2].write('mask for inpainting')
            cols[2].image(pin_ref_mask_V2)
            cols[3].write('generated image(text+redux)')
            cols[3].image(gen_img_V2)
        # cols[5].write('generated image(updated mask)')
        # cols[5].image(gen_img_V2)
        # cols[6].write('generated image(updated mask)')
        # cols[6].image(gen_img_V2_ref)
        # if gen_img_4:
